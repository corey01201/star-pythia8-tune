...
# This Python script was auto-generated using YODA v2.1.0.
# Analysis object: /STAR_2003_I631869/d01-x01-y01
# Timestamp: 07-10-2025 (18:42:21)
...
ax_xLabel = r'Leading $E_\pi$ (GeV)'
ax_yLabel = r'$E\frac{d^3\sigma}{dp^3}$ ($\mu b c^3/GeV^2$)'
ax_title  = r'Inclusive $\pi^0$ Cross Section vs Leading $E_\pi$'
ax_xScale = 'linear'
ax_yScale = 'linear'
xLims = (0.3, 10.0)
yLims = (0.11, 1.0)
...
ratio0_ax.yaxis.set_major_locator(mpl.ticker.MultipleLocator(0.4))
ratio0_ax.set_ylim(0.0, 2.0)
...
exec(open(prefix+'d01-x01-y01__data.py').read(), dataf)
...
for label, yvals in dataf['ratio0_yvals'].items():
    ...
    if styles[label]['ratio0_histstyle']: # plot as histogram
        xpos = dataf['xedges']['pp200GeV.yoda'] if styles[label]['ratio0_drawstyle'] else dataf['xpoints']['pp200GeV.yoda']
    ...
    tmp = ratio0_ax.errorbar(dataf['xpoints']['pp200GeV.yoda'], yvals,
    ...
    for varlabel in dataf['ratio0_variation_vals'].keys():
        if varlabel.startswith(label):
            ratio0_ax.plot(dataf['xedges']['pp200GeV.yoda'],
